...
class LammpsSimulation(object):
    ...
    def unique_pair_terms(self):
        ...
        table_str = ""
        if len(list(set(pot_names))) > 1 or (any(['buck' in i for i in list(set(pot_names))])) or (self.options.pair_write) :
    ...
    def assign_force_fields(self):
        ...
        attr = {'graph':self.graph, 'cutoff':self.options.cutoff, 'h_bonding':self.options.h_bonding,
                'keep_metal_geometry':self.options.fix_metal, 'bondtype':self.options.dreid_bond_type,'qeq':self.options.qeq}
    ...
    def write_lammps_files(self):
        ...
        inpfile.close()
        if self.options.qeq:
            qeq_str = self.construct_qeq_file() 
            qeqfile = open("param.qeq.%s"%self.name, 'w')
            qeqfile.writelines(qeq_str)
            qeqfile.close()

        print("files created!")

    def construct_qeq_file(self):
        string = ""
        if(len(self.unique_atom_types.keys()) > 0):
            for key in sorted(self.unique_atom_types.keys()):
                ...
                electroneg, self_see, type = unq_atom['Xi'], unq_atom['Hard'], unq_atom['force_field_type']
                string += "%i %8.4f %8.4f 0.0 0.0 0.0 # %s\n"%(key, electroneg, self_see, type)
...
#             string += "%19.6f %10.6f %10.6f %s %s %s\n"%(self.cell.xy, self.cell.xz, self.cell.yz, "xy", "xz", "yz")
    ...
    def construct_input_file(self):
        ...
        if self.options.qeq:
            qeqfile = open("param.qeq.%s"%self.name, 'w')
            id = self.fixcount()
            inp_str += "\n%-15s %s 1 12 1.0e-10 1000 %s\n"%("fix", "%i all qeq/point"%id,"param.qeq.%s"%self.name)
            inp_str += "run 0\n"
            inp_str += "unfix %i\n\n"%(id)
            inp_str += "%-15s %s\n"%("dump","%s_qeq all custom 1 %s_q element xs ys zs q"%
                                (self.name, self.name))
            inp_str += "%-15s %s\n"%("dump_modify", "%s_qeq element %s"%(
                                     self.name, 
                                     " ".join([self.graph.node[self.unique_atom_types[key]]['element'] 
                                                for key in sorted(self.unique_atom_types.keys())])))
            inp_str += "run 0\n"
        ...
        if (self.options.minimize):
            box_min = "tri"
            min_style = "cg"
            min_eval = 2e-11  # HKUST-1 will not minimize past 1e-11
        ...
        if (self.options.reldef):
            ...
            min_style = "cg"
            min_eval = 1e-9  # HKUST-1 will not minimize past 1e-11
